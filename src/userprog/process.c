...
process_execute (const char *command) 
{
  ...
  char  *save_ptr;
  const char *file_name = strtok_r (command, " ", &save_ptr);
  /* Create a new thread to execute FILE_NAME. */
   ...
   // check if there is parent waiting for it and return the status to the kernal    
    ...
    child->is_waited_by_parent = true;
    child->twice =0;
    list_push_back(&thread_current()->children,&child->child_element);
    sema_down(&thread_current()->child_semaphore);
    
  if(thread_current()->load_status==-1){
     return TID_ERROR;
  }
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
  ...
  const char *file_name = strtok_r (command, " ", &save_ptr);
   if(success){
        get_thread_by_tid(thread_current()->parent_tid)->load_status=1;
   }
   else{
        get_thread_by_tid(thread_current()->parent_tid)->load_status=-1;
   }
   sema_up(&get_thread_by_tid(thread_current()->parent_tid)->child_semaphore);
   /* If load failed, quit. */
...
load (const char *command, void (**eip) (void), void **esp) 
{
  ...
  success = setup_stack (esp,fn_copy);
  /* Set up stack. */
  if (!success)
    goto done;
      ...
      *esp -= strlen(args[j]) + 1;
      ...
      argv[j]=*esp;
        ...
        *esp = *esp - sizeof(uint8_t);
        memcpy(*esp,&word,sizeof(uint8_t));
        int last=0;
        *esp = *esp - sizeof(char*);
        memcpy(*esp,&last,sizeof(char*));
        for(int j=k-1;j>=0;j--){
          *esp = *esp - sizeof(char*);
        ...
        int first=*esp;
        *esp = *esp - sizeof(char**);
        memcpy(*esp,&first,sizeof(char**));
        *esp = *esp - sizeof(int);
        memcpy(*esp,&k,sizeof(int));
        *esp = *esp -sizeof(void*);
        memcpy(*esp,&last,sizeof(void*)); 
        free(argv); 
